...
const Landing = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 to-amber-800">
      ...
      {/* Hero Section */}
      <section id='Home' className="min-h-screen flex flex-col items-center justify-center p-6 pt-20 sm:pt-24 text-center">
        ...
        //   onClick={handleGetStarted}
          className="bg-purple-600 text-white px-6 sm:px-8 py-3 rounded-full hover:bg-purple-700 hover:shadow-[0_0_5px_#7c3aed] transition-all duration-300 text-lg font-semibold"
        >
          Get Started
        </button>
        </a>
      ...
      <section id='features' className="py-16 px-6 bg-gray-900/30">
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-12">Why TaskTribe?</h2>
      ...
      <section id='about' className="py-16 px-6 bg-gray-900/50">
        <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-12">What Our Tribe Says</h2>
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6">
          {[
            {
              quote: 'TaskTribe made our team unstoppable. We complete tasks faster together!',
              author: 'Sarah, Team Lead',
            },
            {
              quote: 'I love how TaskTribe brings us closer. Every task feels like a group win!',
      ...
      <footer className="py-8 px-6 bg-gray-900/70 text-center relative overflow-hidden">
        ...
        <div className="flex justify-center gap-4 mb-4">
          {['Twitter', 'Discord', 'Email'].map((link, index) => (
            <a
              key={index}
              href={`#${link.toLowerCase()}`}
              className="text-gray-300 hover:text-white hover:shadow-[0_0_5px_#7c3aed] transition-all duration-300"
            >
              {link}
            </a>
          ))}
